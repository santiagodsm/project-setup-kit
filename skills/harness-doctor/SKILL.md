...
- The orchestrator must write a fix-story line — verbatim finding text plus `source: <report> #3` — but never reads gate reports, and no gate's return contract carries the finding **number** or a pasteable finding text. Related: acceptance criteria whose only home is a **gitignored** report — load-bearing content in an untracked file is lost on a fresh clone.
...
Then check the inverse — **excess**. A return contract or dispatch instruction that carries what a named file already holds: a return that includes the brief's or report's contents, a dispatch told to paste a file an agent could read by path. **MAJOR.** Nothing fails when this rots in — the work still succeeds — the orchestrator's context just stops being flat, and sessions quietly shorten. Sufficiency breaks loudly at the next hop; excess never breaks at all, which is exactly why it must be audited. (Bounded inline values the consumer cannot get from disk — a pasted gate finding, a model override, ask wording — are correct, not excess.)
...
- **Every dispatch has a model.** Each agent file declares `model:`; the gate/utility dispatches in the orchestrator skill name one explicitly (judgment gates strong, mechanical gates mid, commit-only/notify chores cheap). A dispatch with no model silently inherits the session's — chores overpay while nothing guarantees the judgment work. MAJOR.
