...
| **Fix-stories + the `source:` pointer** | No pointer to resolve, no report for the scoper to open | **Repair stories.** The orchestrator records `regression-run`'s numbered finding **verbatim** on the repair story's state-file `finding:` line (the tracked home — the report is gitignored and context does not survive a handoff) and pastes it straight into the scoper's dispatch prompt. The finding is already in its context, so a disk pointer the scoper resolves would be indirection for nothing. The anti-invention contract survives: the scoper **refuses to scope rather than invent AC** if the dispatch prompt carries no concrete finding. |
